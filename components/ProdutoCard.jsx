  ...
  return ( 
    <div>
      <div className="ProdutoCard divImage full relative min-h-[70vw] lg:min-h-[24vw]" style={{ backgroundImage: `url(${produto.images[0]?.url})`}} onClick={() => {
        if (redirectImage) {
          window.location.assign((self) ? ('/produtos/' + produto.id) : ((link === '/produtos/kbspktesouras') ? '/produtos/kbspk' : link));
        }
      }}>
        <Link className="text-2xl" href={(self) ? ('/produtos/' + produto.id) : ((link === '/produtos/kbspktesouras') ? '/produtos/kbspk' : link)}>
          {children}
        </Link>
      </div>
